...
export default function MobileMenu({ navLinks }) {
  ...
  return (
    <>
      <Button variant="secondary" onClick={() => setOpen(true)} aria-label="Open menu" className={iconButtonClass}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[999] bg-black/90 backdrop-blur-2xl flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-10">
              {navLinks.map((item, index) => (
                <motion.div key={item.label} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }}>
                  <Link to={item.href} onClick={() => setOpen(false)} className="text-3xl font-bold uppercase tracking-[0.2em] text-neutral-200 hover:text-amber-400 transition-all duration-300">
                    {item.label}
                  </Link>
                </motion.div>
              ))}
            ...
            <div className="p-8 text-center border-t border-white/10">
              <p className="text-neutral-500 uppercase tracking-[0.25em] text-xs">Enter The World Of Cinema</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

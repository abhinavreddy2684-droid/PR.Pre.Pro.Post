  ...
  return (
    <AuthLayout eyebrow="Talent Network / Account recovery" title={<>FIND<br />YOUR<br />WAY BACK.</>} description="Enter the email attached to your account. We’ll send a secure link to create a new password.">
      {sent ? (
        ...
        <>
          <div className="mb-10"><p className="mb-2 text-[9px] uppercase tracking-[0.32em] text-amber-300/55">Scene 03 / Recovery</p><h2 className="font-sans text-4xl font-black leading-none tracking-[-0.035em] text-white">RESET PASSWORD</h2><p className="mt-3 text-sm text-white/35">We’ll help you get back into your account.</p></div>
          ...
          <form onSubmit={handleSubmit} className="space-y-7">
            <label className="block"><span className="mb-3 block text-[9px] uppercase tracking-[0.25em] text-white/40">Email address</span><input className={inputClass} type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" placeholder="you@example.com" required /></label>
          ...
          <p className="mt-9 text-center text-xs text-white/30">Remembered your password? <Link to="/login" className="rounded-full px-2 py-1 text-amber-300/80 transition hover:bg-amber-400/[0.05] hover:text-amber-300">Sign in</Link></p>
        </>
      )}
    </AuthLayout>
  );

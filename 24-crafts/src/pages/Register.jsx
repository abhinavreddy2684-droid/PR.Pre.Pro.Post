...
export default function Register() {
  ...
  return (
    <AuthLayout eyebrow="Talent Network / Create account" title={<>YOUR<br />STORY<br />STARTS.</>} description="Create your account and move straight into talent onboarding. You can shape the profile around your craft as your work grows." footer={<p className="text-center text-xs text-white/35">Already have an account? <Link to={`/login?next=${encodeURIComponent(next)}`} className="font-medium text-amber-300 transition hover:text-amber-200">Sign in</Link></p>}>
      <div className="mb-10"><p className="mb-2 text-[9px] uppercase tracking-[0.32em] text-amber-300/55">Scene 02 / Create</p><h2 className="font-sans text-4xl font-black leading-none tracking-[-0.035em] text-white">CREATE ACCOUNT</h2><p className="mt-3 text-sm text-white/35">Join the Talent Network.</p></div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <label className="block"><span className="mb-3 block text-[9px] uppercase tracking-[0.25em] text-white/40">Full name</span><input className={inputClass} value={form.name} onChange={update("name")} autoComplete="name" placeholder="Your name" required /></label>
        <label className="block"><span className="mb-3 block text-[9px] uppercase tracking-[0.25em] text-white/40">Email address</span><input className={inputClass} type="email" value={form.email} onChange={update("email")} autoComplete="email" placeholder="you@example.com" required /></label>
        <label className="block"><span className="mb-3 block text-[9px] uppercase tracking-[0.25em] text-white/40">Password</span><input className={inputClass} type="password" value={form.password} onChange={update("password")} autoComplete="new-password" placeholder="Create a password" minLength={8} required /></label>
        <label className="block"><span className="mb-3 block text-[9px] uppercase tracking-[0.25em] text-white/40">Confirm password</span><input className={inputClass} type="password" value={form.confirmPassword} onChange={update("confirmPassword")} autoComplete="new-password" placeholder="Repeat your password" minLength={8} required /></label>
        <div className="flex items-start gap-2 rounded-full bg-white/[0.02] px-4 py-3 text-[11px] leading-5 text-white/30"><Check size={14} className="mt-0.5 shrink-0 text-amber-400" />Your account can be used to continue into talent onboarding.</div>

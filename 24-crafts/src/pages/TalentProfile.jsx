...
function MediaCard({ item, talent }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-[28px] bg-[#111111] shadow-[0_18px_60px_rgba(0,0,0,0.22)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_28px_90px_rgba(0,0,0,0.42)]">
      <div className="relative aspect-[16/11] overflow-hidden bg-neutral-900">
        ...
        <div className="absolute left-4 top-4 flex items-center gap-2 rounded-full border border-white/15 bg-black/45 px-3.5 py-2 backdrop-blur-xl"><Icon size={12} className="text-amber-300" /><span className="text-[9px] font-medium uppercase tracking-[0.18em] text-white/75">{item.type}</span></div>
        <span className="absolute right-4 top-4 rounded-full border border-white/10 bg-black/35 px-3 py-2 text-[9px] font-medium tracking-[0.08em] text-white/50 backdrop-blur-xl">{item.year}</span>
        ...
        <div className="absolute inset-x-5 bottom-5 flex items-end justify-end"><span className="rounded-full border border-white/15 bg-black/35 px-3 py-1.5 text-[9px] uppercase tracking-[0.14em] text-white/55 backdrop-blur-xl">View work</span></div>
      </div>
      <div className="p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><h3 className="truncate text-[18px] font-medium tracking-[-0.01em] text-white">{item.title}</h3><p className="mt-2 line-clamp-2 text-[13px] leading-6 text-white/40">{item.description}</p></div><span className="mt-1 shrink-0 text-[10px] font-medium text-white/20">{String(item.id).padStart(2, "0")}</span></div><div className="mt-6 flex items-center justify-between"><span className="text-[9px] uppercase tracking-[0.16em] text-white/25">{talent.craft}</span><span className="inline-flex items-center gap-1.5 text-xs text-white/35 transition-colors group-hover:text-white/55"><Heart size={13} />{item.likes}</span></div></div>
    </article>
  );
}
...
function Experience({ talent }) {
  ...
  return (
    <section className="relative mt-32 scroll-mt-24" id="experience">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-medium uppercase tracking-[0.28em] text-amber-400">02 / Experience</p><h2 className="mt-5 max-w-3xl text-[42px] font-semibold leading-[1.04] tracking-[-0.035em] sm:text-[56px]">The career behind the craft.</h2><p className="mt-5 max-w-xl text-[14px] leading-7 text-white/40">A concise view of the chapters that shaped {talent.name}&apos;s professional work.</p></div><div className="shrink-0 pb-1"><span className="text-3xl font-semibold tracking-[-0.03em] text-amber-200">{String(experienceYears).padStart(2, "0")}+</span><span className="ml-2 text-[9px] font-medium uppercase tracking-[0.16em] text-white/30">years</span></div></div>
      <div className="mt-12 grid gap-5 lg:grid-cols-3">{entries.map((entry, index) => <article key={`${entry.period}-${entry.title}`} className={`group relative overflow-hidden rounded-[30px] bg-gradient-to-br from-white/[0.055] via-white/[0.025] to-transparent p-6 shadow-[0_24px_80px_rgba(0,0,0,0.18)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_30px_100px_rgba(0,0,0,0.32)] ${index === 0 ? "ring-1 ring-amber-300/15" : "ring-1 ring-white/[0.06]"}`}><div className="pointer-events-none absolute -right-20 -top-20 h-44 w-44 rounded-full bg-amber-300/0 blur-3xl transition duration-700 group-hover:bg-amber-300/[0.07]" /><div className="relative flex h-full flex-col"><div className="flex items-center justify-between"><span className={`rounded-full px-3 py-1.5 text-[9px] font-medium ${index === 0 ? "bg-amber-300/10 text-amber-200" : "bg-white/[0.045] text-white/40"}`}>{entry.period}</span><span className="text-[10px] font-medium text-white/15">0{index + 1}</span></div><p className="mt-10 text-[9px] font-medium uppercase tracking-[0.2em] text-amber-300/60">{index === 0 ? "Current chapter" : "Professional chapter"}</p><h3 className="mt-3 text-[25px] font-semibold leading-tight tracking-[-0.025em] text-white">{entry.title}</h3><p className="mt-2 text-[9px] font-medium uppercase tracking-[0.16em] text-white/30">{entry.company}</p><p className="mt-6 flex-1 text-[14px] leading-7 text-white/40">{entry.description}</p><div className="mt-8 flex items-center justify-between"><span className="rounded-full bg-white/[0.035] px-3 py-1.5 text-[9px] font-medium uppercase tracking-[0.14em] text-white/35">{talent.craft}</span><span className="text-[9px] font-medium uppercase tracking-[0.14em] text-white/15 transition group-hover:text-amber-300/60">View chapter</span></div></div></article>)}</div>
    </section>
  );
}
    ...
    <section id="availability" className="relative mt-28 scroll-mt-24">
      ...
      <div className="relative overflow-hidden rounded-[36px] bg-gradient-to-br from-white/[0.055] via-white/[0.025] to-amber-300/[0.025] p-7 shadow-[0_30px_100px_rgba(0,0,0,0.22)] sm:p-10 lg:p-12">
        <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:justify-between lg:gap-16">
          <div className="max-w-md lg:pb-2"><div className="flex items-center gap-3"><CalendarDays size={15} className="text-amber-300" /><p className="text-[10px] font-medium uppercase tracking-[0.28em] text-amber-400">03 / Availability</p></div><h2 className="mt-4 text-[32px] font-semibold leading-tight tracking-[-0.025em] sm:text-[38px]">Find a day that works.</h2><p className="mt-4 text-[14px] leading-6 text-white/40">Yellow is open, green needs confirmation, and muted dates are unavailable.</p><div className="mt-6 flex flex-wrap gap-x-5 gap-y-3 text-[9px] font-medium uppercase tracking-[0.14em] text-white/35"><span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-amber-300" /> Open</span><span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-emerald-400" /> Tentative</span><span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-white/20" /> Unavailable</span></div>{selectedMeta && <div className="mt-7 inline-flex items-center gap-3 rounded-full bg-black/20 px-4 py-2.5"><span className={`h-2 w-2 rounded-full ${selectedMeta.dot}`} /><span className="text-xs text-white/60">{monthLabel} {selectedDay}</span><span className={`text-[9px] font-medium uppercase tracking-[0.12em] ${selectedMeta.text}`}>{selectedMeta.label}</span></div>}</div>
          <div className="w-full max-w-[410px] rounded-[26px] bg-black/20 p-5 sm:p-6"><div className="flex items-center justify-between"><button type="button" onClick={() => shiftMonth(-1)} aria-label="Previous month" className="grid h-9 w-9 place-items-center rounded-full bg-white/[0.035] text-white/40 transition hover:bg-white/[0.07] hover:text-white"><ChevronLeft size={15} /></button><div className="text-center"><p className="text-sm font-medium text-white/85">{monthLabel}</p><p className="mt-0.5 text-[8px] font-medium uppercase tracking-[0.18em] text-white/25">Select a date</p></div><button type="button" onClick={() => shiftMonth(1)} aria-label="Next month" className="grid h-9 w-9 place-items-center rounded-full bg-white/[0.035] text-white/40 transition hover:bg-white/[0.07] hover:text-white"><ChevronRight size={15} /></button></div><div className="mt-5 grid grid-cols-7 gap-1 text-center text-[8px] font-medium uppercase tracking-[0.1em] text-white/25">{["S","M","T","W","T","F","S"].map((day, index) => <span key={`${day}-${index}`} className="py-1">{day}</span>)}</div><div className="mt-1 grid grid-cols-7 gap-1">{Array.from({ length: days.firstDay }).map((_, index) => <span key={`blank-${index}`} className="h-9" />)}{Array.from({ length: days.daysInMonth }, (_, index) => index + 1).map((day) => { const status = getStatus(day); const meta = statusMeta[status]; const selected = selectedDay === day; return <button key={day} type="button" disabled={status === "unavailable"} aria-label={`${monthLabel} ${day}: ${meta.label}`} title={status === "unavailable" ? "Unavailable — already booked" : `${meta.label}${status === "tentative" ? " — needs confirmation" : ""}`} onClick={() => setSelectedDay(day)} className={`group relative h-9 rounded-xl text-xs font-medium transition ${status === "available" ? "text-amber-100 hover:bg-amber-300/10 hover:text-amber-200" : status === "tentative" ? "text-emerald-200 hover:bg-emerald-400/10" : "cursor-not-allowed text-white/15 line-through decoration-white/20"} ${selected ? `bg-white/[0.07] ring-1 ${meta.ring}` : ""} disabled:hover:bg-transparent`}><span>{day}</span><span className={`absolute bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full ${meta.dot}`} />{status === "unavailable" && <span className="pointer-events-none absolute inset-0 grid place-items-center text-[11px] text-white/15 opacity-0 transition-opacity group-hover:opacity-100">×</span>}</button>; })}</div></div>
        </div>
      </div>
    </section>
  );
...
export default function TalentProfile() {
  ...
  return <div className="min-h-screen overflow-hidden bg-[#090909] text-white"><Navbar /><main className="px-6 pb-24 pt-32"><div className="mx-auto max-w-7xl"><Link to={`/talent?craft=${encodeURIComponent(talent.craft)}`} className="inline-flex items-center gap-2 text-sm text-neutral-500 transition-colors hover:text-amber-400"><ArrowLeft size={16} /> Back to {talent.craft}</Link><section className="mt-10 grid items-stretch gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16"><div className="relative min-h-[560px] overflow-hidden rounded-[2.5rem] border border-white/10 bg-neutral-900"><img src={talent.image} alt={talent.name} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent" /><span className="absolute bottom-6 left-6 rounded-full bg-amber-400 px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-black">{talent.availability}</span></div><div className="flex flex-col justify-center py-4 lg:py-10"><p className="text-xs font-medium uppercase tracking-[0.3em] text-amber-400">{talent.craft}</p><h1 className="mt-5 max-w-4xl text-[52px] font-semibold leading-[0.96] tracking-[-0.04em] md:text-[76px]">{talent.name}</h1><div className="mt-7 flex flex-wrap gap-x-6 gap-y-3 text-[14px] text-neutral-400"><span className="inline-flex items-center gap-2"><MapPin size={16} /> {talent.location}</span><span className="inline-flex items-center gap-2"><Star size={16} className="text-amber-400" /> 4.9 rating</span><span className="inline-flex items-center gap-2"><BriefcaseBusiness size={16} /> {talent.experience}</span></div><p className="mt-8 max-w-2xl text-[16px] leading-8 text-neutral-400">{talent.bio}</p><div className="mt-8 flex flex-wrap gap-2">{talent.genres.map((genre) => <span key={genre} className="rounded-full border border-amber-500/20 bg-amber-500/10 px-4 py-2 text-[13px] text-amber-300">{genre}</span>)}</div><div className="mt-10 flex flex-wrap gap-4"><Button className="inline-flex items-center gap-3">Hire Talent <ArrowRight size={18} /></Button><Button variant="secondary" onClick={() => document.getElementById("work")?.scrollIntoView({ behavior: "smooth", block: "start" })}>View Portfolio</Button></div></div></section><Gallery talent={talent} /><Experience talent={talent} /><Availability /></div></main><Footer /></div>;
}

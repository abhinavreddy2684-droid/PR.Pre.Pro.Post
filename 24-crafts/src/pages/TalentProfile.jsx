  ...
  return (
    <article className="group relative overflow-hidden rounded-[28px] border border-white/[0.09] bg-[#111111] shadow-[0_18px_60px_rgba(0,0,0,0.22)] transition duration-500 hover:-translate-y-1 hover:border-amber-300/25 hover:shadow-[0_24px_80px_rgba(0,0,0,0.4)]">
      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4"><div className="min-w-0"><h3 className="truncate text-[19px] font-medium tracking-tight text-white">{item.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-white/40">{item.description}</p></div><span className="mt-1 shrink-0 font-mono text-[10px] text-white/25">#{String(item.id).padStart(2, "0")}</span></div>
...
function Experience({ talent }) {
  ...
  return (
    <section className="mt-32 scroll-mt-24" id="experience">
      <div className="border-b border-white/[0.08] pb-8"><p className="text-[10px] uppercase tracking-[0.32em] text-amber-400">02 / Experience</p><h2 className="mt-5 text-4xl font-semibold tracking-tight sm:text-6xl">Professional experience.</h2></div>
      <div className="mt-2">{entries.map((entry, index) => <article key={`${entry.period}-${entry.title}`} className="grid gap-4 border-b border-white/[0.08] py-8 sm:grid-cols-[150px_1fr] sm:gap-10 sm:py-10"><p className="font-mono text-xs text-amber-300/75">{entry.period}</p><div className="grid gap-3 md:grid-cols-[0.8fr_1.2fr] md:gap-10"><div><h3 className="text-xl font-medium tracking-tight">{entry.title}</h3><p className="mt-1 text-xs uppercase tracking-[0.15em] text-white/30">{entry.company}</p></div><p className="max-w-2xl text-sm leading-7 text-white/40">{entry.description}</p></div><span className="hidden font-mono text-[9px] text-white/15 sm:block">0{index + 1}</span></article>)}</div>
    </section>
  );
}
...
function Availability() {
  ...
  const getStatus = (day) => {
    ...
    const score = (day * 7 + date.getDay() * 3) % 12;
    if (score >= 9) return "available";
    if (score >= 6) return "tentative";
    return "unavailable";
  };

  const statusMeta = {
    available: { label: "Available", short: "Open", dot: "bg-amber-300", text: "text-amber-200", ring: "ring-amber-300/40" },
    tentative: { label: "Tentative", short: "Needs confirmation", dot: "bg-emerald-400", text: "text-emerald-300", ring: "ring-emerald-400/35" },
    unavailable: { label: "Unavailable", short: "Booked", dot: "bg-white/20", text: "text-white/30", ring: "ring-white/10" },
  };
  ...
  return (
    <section id="availability" className="mt-28 scroll-mt-24">
      <div className="flex flex-col gap-8 border-y border-white/[0.08] py-10 sm:py-12 lg:flex-row lg:items-center lg:justify-between lg:gap-16">
        <div className="max-w-md lg:pb-2">
          <div className="flex items-center gap-3"><CalendarDays size={15} className="text-amber-300" /><p className="text-[10px] uppercase tracking-[0.3em] text-amber-400">03 / Availability</p></div>
          <h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">Find a day that works.</h2>
          <p className="mt-4 text-sm leading-6 text-white/40">A compact view of this talent&apos;s schedule. Yellow is open, green needs confirmation, and muted dates are unavailable.</p>
          <div className="mt-6 flex flex-wrap gap-x-5 gap-y-3 text-[9px] uppercase tracking-[0.15em] text-white/35">
            <span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-amber-300" /> Open</span>
            <span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-emerald-400" /> Tentative</span>
            <span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-white/20" /> Unavailable</span>
          </div>
          {selectedMeta && <div className="mt-7 inline-flex items-center gap-3 rounded-full border border-white/[0.08] bg-white/[0.025] px-4 py-2.5"><span className={`h-2 w-2 rounded-full ${selectedMeta.dot}`} /><span className="text-xs text-white/60">{monthLabel} {selectedDay}</span><span className={`text-[9px] uppercase tracking-[0.14em] ${selectedMeta.text}`}>{selectedMeta.label}</span></div>}
        </div>

        <div className="w-full max-w-[410px] self-start rounded-[26px] border border-white/[0.09] bg-[#111111] p-5 shadow-[0_20px_70px_rgba(0,0,0,0.2)] sm:p-6 lg:self-auto">
          <div className="flex items-center justify-between"><button type="button" onClick={() => shiftMonth(-1)} aria-label="Previous month" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-white/40 transition hover:border-white/20 hover:text-white"><ChevronLeft size={15} /></button><div className="text-center"><p className="text-sm font-medium text-white/85">{monthLabel}</p><p className="mt-0.5 text-[8px] uppercase tracking-[0.2em] text-white/25">Select a date</p></div><button type="button" onClick={() => shiftMonth(1)} aria-label="Next month" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-white/40 transition hover:border-white/20 hover:text-white"><ChevronRight size={15} /></button></div>
          <div className="mt-5 grid grid-cols-7 gap-1 text-center text-[8px] uppercase tracking-[0.12em] text-white/25">{["S","M","T","W","T","F","S"].map((day, index) => <span key={`${day}-${index}`} className="py-1">{day}</span>)}</div>
          <div className="mt-1 grid grid-cols-7 gap-1">{Array.from({ length: days.firstDay }).map((_, index) => <span key={`blank-${index}`} className="h-9" />)}{Array.from({ length: days.daysInMonth }, (_, index) => index + 1).map((day) => { const status = getStatus(day); const meta = statusMeta[status]; const selected = selectedDay === day; return <button key={day} type="button" disabled={status === "unavailable"} aria-label={`${monthLabel} ${day}: ${meta.label}`} title={status === "unavailable" ? "Unavailable — already booked" : `${meta.label}${status === "tentative" ? " — needs confirmation" : ""}`} onClick={() => setSelectedDay(day)} className={`group relative h-9 rounded-xl text-xs font-mono transition ${status === "available" ? "text-amber-100 hover:bg-amber-300/10 hover:text-amber-200" : status === "tentative" ? "text-emerald-200 hover:bg-emerald-400/10" : "cursor-not-allowed text-white/15 line-through decoration-white/20"} ${selected ? `bg-white/[0.07] ring-1 ${meta.ring}` : ""} disabled:hover:bg-transparent`}><span>{day}</span><span className={`absolute bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full ${meta.dot}`} />{status === "unavailable" && <span className="pointer-events-none absolute inset-0 grid place-items-center text-[11px] text-white/15 opacity-0 transition-opacity group-hover:opacity-100">×</span>}</button>; })}</div>
        </div>
      </div>
    </section>
  );
